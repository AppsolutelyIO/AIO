...
When used as a pre-commit check (quick scan of staged changes only):

### Indicators to Detect

- `API_KEY`, `SECRET`, `PASSWORD`, `PRIVATE_KEY`, `TOKEN`
- `AWS_SECRET_ACCESS_KEY`, `AWS_ACCESS_KEY_ID`
- Bearer tokens, SSH private keys, PGP keys
...
- Config files containing inline credentials

### Rules

- **Abort commit if secrets are detected.**
- Report the file, line number, and matched pattern.
