...
description: >
  Code quality enforcement — write-time auto-formatting, linting, and
  Claude-powered fixes via hooks, plus pre-commit formatting, lint gates,
  and complexity checks. Activates when preparing commits, running quality
  checks, or when the user mentions formatting, linting, code complexity,
  or code quality.
origin: community
---
...
When preparing code for commit (without Plankton hooks), run these checks manually.

### Format Gate

Run the project formatter on staged files:

| Stack | Command | Fallback |
|-------|---------|----------|
| JS/TS | `npm run format` / `pnpm format` | `npx prettier --write .` |
| PHP | `vendor/bin/pint --dirty --format agent` | — |
| Python | `ruff format` | `black .` |
| Go | `gofmt -w .` | — |
| Rust | `cargo fmt` | — |

1. Run the formatter on staged files only.
2. If files change after formatting, stage them: `git add <changed files>`.
3. Report which files were reformatted.

### Lint Gate

Run lint tools to catch errors and style violations:

| Stack | Command |
|-------|---------|
| JS/TS | `npm run lint` or `npx eslint .` |
| PHP | `vendor/bin/phpstan analyse` (if available) |
| Python | `ruff check` |
| Go | `golangci-lint run` |
| Rust | `cargo clippy` |

- **Abort the commit if lint fails** with errors.
- Warnings may proceed but must be reported.
- Prefer project-configured lint commands over generic ones.

### Code Complexity Check

Detect excessive complexity in staged changes:
...
Available tools: ESLint `complexity` rule (JS/TS), `radon` (Python), `phpmd` (PHP), `gocyclo` (Go).

- Recommend refactoring if limits exceeded.
- Do not block commit for complexity alone — warn and suggest improvements.
