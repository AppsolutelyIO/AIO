...
When used as a pre-commit check (quick dependency audit):
...
- Warn if vulnerabilities detected (do not block commit).
- Report severity levels (critical, high, medium, low).
- Suggest `npm audit fix` or equivalent when auto-fixable.
