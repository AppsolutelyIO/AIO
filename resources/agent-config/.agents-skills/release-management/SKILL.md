...
description: >
  Bump version numbers and update changelog based on commit types.
  GitHub release workflow with semantic versioning, changelogs, and release automation using gh CLI.
  Activates when preparing releases, bumping versions, updating changelogs,
  or when the user mentions release, version, changelog, or semver.
context: fork
version: 1.0.0
author: OrchestKit
...
- User asks about versioning or release preparation
...
| `refactor`, `perf`, `style` | Patch (0.0.x) |
| `docs`, `test`, `ci`, `chore` | No bump |

### Rules

- Follow existing versioning convention in the project.
- Ask for confirmation before bumping major versions.
- Update all version files consistently.
...
If `CHANGELOG.md` exists, append a new entry.
...
### Other
- refactoring, performance, docs changes
```

#### Rules

- Place newest entry at the top (below header).
- Use the same style as existing entries.
- Link to commit hashes or PR numbers when available.
- Group changes by type (Features, Fixes, Breaking Changes, Other).
...
- [ ] Version numbers updated in package.json/composer.json/pyproject.toml
...
7. **Confirm before major bumps** - Always ask for confirmation before major version changes
