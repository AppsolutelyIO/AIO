...
## Pre-Commit Gate Mode

When used as a pre-commit check (quick SAST on staged code):
...
| Python | `bandit` |
| PHP | `phpstan-security` (if available) |
| Go | `gosec` |

### Rules

- **Abort commit on high-severity vulnerabilities.**
- Report findings with file, line, and description.
- Medium/low severity: warn but allow commit to proceed.
