...
description: >
  Split staged changes into logical commits, generate Conventional Commit
  messages, and execute commits. Activates when committing code, splitting
  commits, generating commit messages, or when the user mentions commit,
  commit message, or conventional commits. Provides guidance on commit message
  structure, types, scopes, and best practices.
---
...
When committing staged changes, analyze the diff and split into logical commits when:

- Multiple modules/features changed independently
- Feature code mixed with refactoring
- Migration mixed with business logic
- Formatting changes mixed with feature code
- Test additions mixed with implementation
...
Infer scope from directory paths of changed files:
...
| `themes/` | `theme` |
| `resources/page-builder/` | `page-builder` |

If changes span multiple scopes, use the primary scope or omit it.
...
**Example (bullet points for non-trivial changes):**
```
feat(auth): add refresh token rotation support

- implement token rotation on each refresh
- validate token expiration before rotation
- update login response payload with new token
```
...
1. Stage only the files belonging to this group.
...
- All commits must appear as written by an experienced human developer.
