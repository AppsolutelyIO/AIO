...
<head>
    ...
    <title>@if(! empty($header)){{ $header }} | @endif {{ Appsolutely\AIO\Admin::title() }}</title>

    @if(! config('admin.disable_no_referrer_meta'))
        <meta name="referrer" content="no-referrer"/>
    @endif

    @if(! empty($favicon = Appsolutely\AIO\Admin::favicon()))
        <link rel="shortcut icon" href="{{$favicon}}">
    @endif

    {!! admin_section(Appsolutely\AIO\Admin::SECTION['HEAD']) !!}

    {!! Appsolutely\AIO\Admin::asset()->headerJsToHtml() !!}
