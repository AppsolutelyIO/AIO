/**
 * --------------------------------------------
 * AdminLTE CardWidget.ts
 * License MIT
 * --------------------------------------------
 */

interface CardWidgetSettings {
...
const CardWidget = (($: JQueryStatic) => {
  ...
  const Default: CardWidgetSettings = {
  ...
  class CardWidget {
    ...
    private _settings: CardWidgetSettings

    constructor(element: JQuery, settings: CardWidgetSettings) {
      ...
      this._settings = $.extend({}, Default, settings) as CardWidgetSettings
    }
    ...
    minimize(): void {
      ...
      this._parent.css('cssText', 'height:' + (this._parent[0] as HTMLElement).style.height + ' !important;' +
        'width:' + (this._parent[0] as HTMLElement).style.width + ' !important; transition: all .15s;'
    ...
    _init(card: JQuery): void {
      this._parent = card

      $(this as unknown as HTMLElement).find(this._settings.collapseTrigger).click(() => {
        this.toggle()
      })

      $(this as unknown as HTMLElement).find(this._settings.maximizeTrigger).click(() => {
        this.toggleMaximize()
      })

      $(this as unknown as HTMLElement).find(this._settings.removeTrigger).click(() => {
        this.remove()
      })
    }
    ...
    static _jQueryInterface(this: JQuery, config: string | Record<string, unknown>): void {
      let data = $(this).data(DATA_KEY) as CardWidget | undefined
      const _options = $.extend({}, Default, $(this).data()) as CardWidgetSettings
