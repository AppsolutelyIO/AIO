let tpl =
        '<div class="aio-loading d-flex items-center align-items-center justify-content-center pin" style="{style}">{svg}</div>',
...
class Loading {
    ...
    constructor(AIO: AIOInstance, options?: Partial<LoadingOptions>) {
        ...
        _this.$container = $(merged.container as any);
        ...
        content = $(
            tpl
                ...
                .replace(
                    '{style}',
                    `${defStyle}background:${merged.background};z-index:${merged.zIndex};${merged.style}`,
                ),
        );
...
function extend(AIO: AIOInstance): void {
    (AIO as Record<string, unknown>).loading = function (
        options?: Partial<FullScreenLoadingOptions> | false,
    ): ReturnType<typeof setTimeout> | Loading | undefined {
        if (options === false) {
        ...
        const merged = $.extend(
            ...
            options,
        ) as FullScreenLoadingOptions;

        var win = $(window),
            $container = $(
                '<div class="aio-loading" style="z-index:' + merged.zIndex + ';width:300px;position:fixed"></div>',
            ),
            shadow = $(
                '<div class="layui-layer-shade aio-loading" style="z-index:' +
                    (merged.zIndex - 2) +
                    '; background-color:' +
                    merged.shade +
                    '"></div>',
        ...
        ($container as any).loading(merged as unknown as Record<string, unknown>);
    };

    ($.fn as any).loading = function (this: JQuery, opt?: Partial<LoadingOptions> | false): JQuery | Loading {
        if (opt === false) {
    ...
    ($.fn as any).buttonLoading = function (this: JQuery, start?: boolean): JQuery {
        let $this = $(this),
        ...
        if (start === false) {
            ...
            return $this
                ...
                .html($this.find('.' + loadingId).html() as string);
        }
        ...
        return $this.addClass('disabled btn-loading').attr('disabled', 'disabled').attr('data-loading', loadingId)
            .html(`
