...
class Loading {
    constructor(AIO, options) {
        options = $.extend({
            container: AIO.config.pjax_container_selector,
            zIndex: 100,
            width: '52px',
            color: AIO.color.dark60,
            background: '#fff',
            style: '',
            svg: LOADING_SVG[0]
        }, options);
...
function extend(AIO) {
    ...
    AIO.loading = function (options) {
        ...
        var win = $(window),
            // 容器
            $container = $('<div class="aio-loading" style="z-index:'+options.zIndex+';width:300px;position:fixed"></div>'),
            // 遮罩层直接沿用layer
            shadow = $('<div class="layui-layer-shade aio-loading" style="z-index:'+(options.zIndex-2)+'; background-color:'+options.shade+'"></div>');
