...
let defaultActions = {
    ...
    delete (action, AIO) {
        ...
        $document.on('click', action, function() {
            ...
            AIO.confirm(lang.delete_confirm, msg, function () {
                AIO.NP.start();
                $.delete({
                    url: url,
                    success: function (response) {
                        ...
                        AIO.handleJsonResponse(response);
                    }
                });
            });
        });
    ...
    // 批量删除按钮初始化
    'batch-delete' (action, AIO) {
        $document.on('click', action, function() {
            let url = $(this).data('url'),
                name = $(this).data('name'),
                redirect = $(this).data('redirect'),
                keys = AIO.grid.selected(name),
                lang = AIO.lang;
    ...
    // 图片预览
    'preview-img' (action, AIO) {
        $document.on('click', action, function () {
            return AIO.helpers.previewImage($(this).attr('src'));
        });
    },
...
export default class DataActions {
    constructor(AIO) {
        let actions = $.extend(defaultActions, AIO.actions()),
            name;
