...
interface TreeOptions {
    ...
    showNextPage: string | boolean;
    pageQueryName: string;
    parentIdQueryName: string;
    depthQueryName: string;
    showIcon: string;
    hideIcon: string;
    loadMoreIcon: string;
}
...
export default class Tree {
    ...
    constructor(Helper: Helper, opts: Partial<TreeOptions>) {
        this.options = $.extend({
            button: null,
            table: null,
            url: '',
    ...
    // 发起请求
    request (page: number, after?: () => void): void {
        var _this = this,
            row = _this.row!,
            key = _this.key!,
            depth = _this.depth!,
        ...
        $.ajax({
            ...
            success: function (resp: string) {
                ...
                var _body = $('<div>'+resp+'</div>'),
                    ...
                    lastPage = parseInt(_body.find('last-page').text()),
                    nextPage = _body.find('next-page').text();

                // 标记子节点行
                _tbody.find('tr').each(function (_, v) {
                    $(v).attr('data-depth', String(depth + 1))
                });
            ...
            error:function(a: JQueryXHR, b: string, c: string){
                after && after();
