...
interface UploaderParent {
    uploader: WebUploaderInstance & { removeFile(file: any): void };
    options: Record<string, unknown>;
    helper: { orderFiles($this: JQuery): void };
    input: { add(id: string): void; delete(id: string): void };
    request: { delete(file: any, callback: (result: any) => void): void };
    status: { switch(val: string, args?: Record<string, unknown>): void };
...
export default class AddUploadedFile {
    ...
    // 渲染已上传文件
    render(file: WebUploaderFile): void {
        let _this = this,
...
`;
        ...
        // 删除操作
        let deleteFile = function (this: HTMLElement): void {
            var fileId = $(this).data('id') as string;
        ...
        $html.find('[data-file-act="preview"]').click(function () {
            var url = $(this).data('url') as string;

            (AIO.helpers as any).previewImage(url);
        });
    ...
    removeFormFile(fileId: string): void {
        ...
        if (uploader && !file.fake) {
            (uploader as any).removeFile(file);
        }

        parent.status.switch('decrOriginalFileNum');
        parent.status.switch('incrFileNumLimit');

        if (! (AIO.helpers as any).len(parent.formFiles) && ! (AIO.helpers as any).len(parent.percentages)) {
            parent.status.switch('pending');
        }
    }

    add(file: WebUploaderFile): void {
        if (!file.serverId || (this.uploader as any).helper.searchUploadedFile(file.serverId) !== -1) {
            return;
        }
