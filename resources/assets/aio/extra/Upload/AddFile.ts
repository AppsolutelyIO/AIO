...
interface UploaderParent {
    uploader: WebUploaderInstance & {
        makeThumb(file: any, callback: (error: string | null, src: string) => void): any;
        removeFile(file: any): void;
    };
    options: Record<string, unknown>;
    lang: { trans(key: string, replacements?: Record<string, unknown>): string };
    helper: { isSupportBase64: boolean; orderFiles($this: JQuery): void };
    input: { delete(id: string): void };
    request: { delete(file: any, callback: () => void): void };
    percentages: Record<string, [number, number]>;
    faildFiles: Record<string, any>;
...
`).appendTo($li);
    ...
    // 显示错误信息
    showError ($li: JQuery, code: string, file: any): void {
        let _this = this,
    ...
    // 显示图片
    showImage($li: JQuery, file: any): void {
        let _this = this,
            uploader = _this.uploader.uploader,
            lang = _this.uploader.lang,
            $wrap = $li.find('p.imgWrap');

        var image = uploader.makeThumb(file, function (error: string | null, src: string) {
            var img: JQuery;
        ...
        try {
            image.once('load', function () {
                ...
                if (! _this.validateDimensions(file)) {
                    AIO.error(lang.trans('dimensions') || 'The image dimensions is invalid.');
    ...
    // 状态变化回调
    resolveStatusChangeCallback($li: JQuery, $btns: JQuery, file: any): (cur: string, prev: string, a: unknown) => void {
        let _this = this,
            parent = _this.uploader;

        return function (cur: string, prev: string, a: unknown) {
            if (prev === 'progress') {
    ...
    // 操作按钮回调
    resolveActionsCallback(file: any): (this: HTMLElement) => void {
        let _this = this,
            parent = _this.uploader,
        ...
        return function (this: HTMLElement) {
            var index = $(this).data('file-act') as string;
            ...
            switch (index) {
                ...
                case 'delete':
                    // 本地删除
                    if ((parent.options as any).removable) {
                    ...
                    break;
                case 'preview':
                    (AIO.helpers as any).previewImage($(this).parent().parent().find('img').attr('src'), null, file.name);
    ...
    // 移除错误信息
    removeError($li: JQuery): void {
        $li.find('.error').remove()
    }

    // 图片宽高验证
    validateDimensions(file: any): boolean {
        let _this = this,
            parent = _this.uploader,
            options = parent.options,
            dimensions = options.dimensions as Record<string, number>,
            width = file._info.width as number,
            height = file._info.height as number,
            isset = (AIO.helpers as any).isset;

        // The image dimensions is invalid.
        if (! parent.isImage() || ! _this.isImage(file) || ! (AIO.helpers as any).len(options.dimensions)) {
            return true;
        }
    ...
    // 判断是否是图片
    isImage (file: any): RegExpMatchArray | null {
