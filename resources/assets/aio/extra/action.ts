    ...
    class Action {
        ...
        constructor(options: Partial<ActionOptions>) {
            this.options = $.extend({
                ...
                html: function (target: JQuery, html: string, data: Record<string, unknown>) { // 处理返回的HTML代码
                    target.html(html);
                },
        ...
        init(): void {
            ...
            $(options.selector!).off(options.event).on(options.event, function (e: JQuery.Event) {
        ...
        resolve(): (result: [Record<string, unknown>, JQuery]) => void {
            ...
            return function (result: [Record<string, unknown>, JQuery]) {
                ...
                AIO.handleJsonResponse(response, {html: options.html, target: target});
            };
        }
